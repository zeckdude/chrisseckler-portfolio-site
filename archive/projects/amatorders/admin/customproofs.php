...
if(!isset($_SESSION['authenticated_oc'])){
	...
	if(isset($_GET['search_submit'])) {
		...
		$search_string = ' WHERE ' . $_GET['searchby'] . '= "' . $_GET['search_input'] . '" AND status = "custom_proof" ';			
...
<div id="container">
           ...
           <script type="text/javascript">
				...
				//alert('The form width is ' + formWidth);

				//alert('The page nav width is ' + pagnavWidth);

				var combinedWidth = formWidth + pagnavWidth + 61;

				//document.write(formWidth);

				$("#search_nav_area").replaceWith('<div id="search_nav_area" style="width: ' + combinedWidth + 'px;">' + $("#search_nav_area").html() + '</div>');

				//alert($("#search_nav_area").html());
			...
			if($custom_cnt < 1) { ?>
            	...
            	<tr id="master">
                    ...
                    <td colspan="10" style="border-bottom: 1px solid #5C7F99;">

                        There are no Custom Proof Orders.
				...
				?>
						...
						<tr id="detail">
							...
							<span style="display: block; position: relative; top: -7px; left: -24px; width: 100%;">
								...
								<a class="button" id="edit_btn" href="upload_custom_proof.php?order_id=<?php echo $row['order_id'] ?>">Upload Custom Proof</a>
                                            ...
                                            <span class="detail_text">
                                                ...
                                                <?php 
													...
													$instr_sql = "SELECT *
														...
														$numRows = $instr_result->num_rows;


                                                    if($row['special_instructions'] == '' && $numRows < 1) { //If there is no original custom correction message and no additional correction messages

                                                        echo 'No Message given';

                                                    } else {
													
															if($row['special_instructions'] != '' && $numRows < 1) { //If there is an original custom correction message and no additional correction messages
