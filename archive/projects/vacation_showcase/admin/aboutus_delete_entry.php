...
session_start();
	...
	if (isset($_GET['travelagent_id']) && !$_POST) { //This is checking that the GET variable 'id' exists in the url and that somebody pushed a form button to get here
	  ...
	  $sql = 'SELECT *
			...
			if ($stmt->prepare($sql)) { 
				...
				$stmt->bind_param('i', $_GET['travelagent_id']); //This is where you specify what GET variable you are using
				
				$stmt->bind_result($id, $name, $company_name, $phone, $email, $cst); //This saves the row's data into variables so you can use them later. They must be in the same order as the SQL query on line 15
	...
	if (array_key_exists('delete', $_POST)) { //If the 'delete' button is clicked, then run this bit of code
	  $sql = 'DELETE FROM travelagents WHERE travelagent_id = ?'; //This tells the database to delete the entry with the id # that is currently being sent from the form
	  $stmt = $conn->stmt_init();
	  if ($stmt->prepare($sql)) {
	...
	if ($deleted == true || array_key_exists('cancel_delete', $_POST) || !isset($_GET['travelagent_id']))  {
	  header('Location: ' . $site_basedir .'admin/aboutus_edit_list.php'); //If any of the top 3 conditions are met, then go to this page
	  exit;
	  }
