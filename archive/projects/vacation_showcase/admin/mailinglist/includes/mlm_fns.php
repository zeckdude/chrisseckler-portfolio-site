...
function store_list($details) {
  if (!filled_out($details))  {
    echo '<div id="adminenter_box"><p>All fields must be filled in.  Try again.</p></div>';
    return false;
  } else  {
...
//I dont think this is needed fr my uses but ill keep it for now
// get all lists
...
// get the list of mail d, but not yet sent
function get_unsent_mail() {
...
echo '<span style="display:block; padding: 20px;">';
  ...
  $from_name = 'Jeff Johnson';

  $from_address = 'jeff@johnson.com';
  ...
  if (!$result) {
    ...
    echo "<p>There is nobody subscribed to that list.</p>";
    return false;
  }

  // include PEAR mail classes
  include('/home5/ideapale/php/Mail.php');
  include('/home5/ideapale/php/Mail/mime.php');
  ...
  if($status == 'STORED') {
    ...
    mail('admin@localhost.com', $subject, $text, "From: test@test.com");
    ...
    while ($subscriber = $result->fetch_row()) {
      if($subscriber[2]=='H') {
		...
		echo $subscriber[1] . " " . $subject . " " . $text;
		$headers = 'From: some@body.com' . "\r\n" .
				    'Reply-To: webmaster@example.com' . "\r\n" .
				    'X-Mailer: PHP/' . phpversion();

        mail($subscriber[1], $subject, $text, $headers) or die ('failed to send message to list');
		//,'From: "'.get_real_name($admin_user).'" <'.$admin_user.'>'
      }
      $count++;
    }
