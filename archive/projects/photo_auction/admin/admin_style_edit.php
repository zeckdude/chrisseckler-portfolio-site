...
error_reporting(E_ALL ^ E_NOTICE ^ E_DEPRECATED);
	...
	//get details of selected record
	 if (isset($_GET['style_id']) && !$_POST) { 
		...
		if ($stmt->prepare($sql)) { 
			...
			$stmt->bind_result($style_id, $name, $desc); 
	...
	if (array_key_exists('update', $_POST)) { //checks that something with the 'update' name attribute is in the $_POST array, which it is once it is clicked. If the user does click on it, the execute the rest of the script
		...
		$sql = 'UPDATE styles
				SET styles.style_name = ?, styles.style_desc = ?
				WHERE styles.style_id = ?';
		...
		$stmt = $conn->stmt_init(); 
		if ($stmt->prepare($sql)) { 
			$stmt->bind_param('ssi', $_POST['name'], $_POST['desc'], $_POST['style_id']); 
	...
	if ($done || !isset($_GET['style_id'])) { //if $_GET['article_id'] is not defined
            ...
            <h1>Edit Style entry </h1>
                ...
                <form id="newphotoform" name="form1" method="post" action="<?php echo $_SERVER['PHP_SELF'];?>" enctype="multipart/form-data">
                    <p>
                        ...
                        <input name="name" type="text" id="style_name" value="<?php echo htmlentities($name); ?>"/></input>
                    </p>
                    
                    <p>
                        <label for="desc">Style Description:</label>
                        <textarea name="desc" id="style_desc"/><?php echo htmlentities($desc); ?></textarea>
                    </p>
                    
                    <p>
                        <input type="submit" name="update" value="Update Style Information" />
