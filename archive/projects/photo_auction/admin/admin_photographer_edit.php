...
error_reporting(E_ALL ^ E_NOTICE ^ E_DEPRECATED);
	...
	if (array_key_exists('update', $_POST)) { //checks that something with the 'update' name attribute is in the $_POST array, which it is once it is clicked. If the user does click on it, the execute the rest of the script
		...
		$sql = 'UPDATE photographers
				SET photographers.photographer_name = ?, photographers.photographer_firstname = ?, photographers.photographer_lastname = ?, photographers.photographer_sitelink = ?
				WHERE photographers.photographer_id = ?';
		...
		$stmt = $conn->stmt_init(); 
		if ($stmt->prepare($sql)) { 
			$stmt->bind_param('ssssi', $photographer_name, $_POST['firstname'], $_POST['lastname'], $_POST['sitelink'], $_POST['photographer_id']); 
	...
	if ($done || !isset($_GET['photographer_id'])) { //if $_GET['article_id'] is not defined
...
<div id="container">
    ...
    <div id="main_content">
        <div id="highest_items">
            <h1>Edit Photo entry </h1>
                
                <form id="newphotoform" name="form1" method="post" action="<?php echo $_SERVER['PHP_SELF'];?>" enctype="multipart/form-data">
                    <p>
                        <label for="firstname">First Name:</label>
                        <input name="firstname" type="text" id="firstname" value="<?php echo htmlentities($firstname); ?>"/></input>
                    </p>
                    
                    <p>
                        <label for="lastname">Last Name:</label>
                        <input name="lastname" type="text" id="lastname" value="<?php echo htmlentities($lastname); ?>"/></input>
                    </p>
    
                     <p>
                        <label for="sitelink">Website URL:</label>
                        <input name="sitelink" type="text" id="lastname" value="<?php echo htmlentities($sitelink); ?>"/></input>
                    </p>
                    
                    <p>
                        <input type="submit" name="update" value="Update Photographer Information" />
