$(document).ready(function() {
	...
	$("#home .text-col h1").fitText(0.9, { minFontSize: '38px', maxFontSize: '50px' });
	$("#home .text-col p").fitText(1.2, { minFontSize: '18px', maxFontSize: '28px' });
	...
	$('#filter-works a').click(function(e){
      ...
      //$('html,body').scrollTo('#projects-container', '#projects-container');
      if($(window).width() < 768) {
        $('html,body').scrollTo('#projects-container', '#projects-container', {
          gap: {y: -85},
  ...
  var title, descr, slidesHtml, elemDataCont, projectName;

  function openProjectBasedOnHash(hash) {
    if (window.location.hash || hash) {
      if(window.location.hash) {
        hash = window.location.hash;
      }

      var projectName = hash.slice(1);
      //alert(projectName);

      if ($('.project-item[data-project="' + projectName + '" ]').length > 0) {
        var project = $('.project-item[data-project="' + projectName + '"]');
        openModal(project);

        $('html,body').scrollTo('#portfolio', '#portfolio');
      }
    }
  }

  $('#feat-more-info-btn').click(function(e) {
    e.preventDefault();

    var hash = $(this).attr('href');
    openProjectBasedOnHash(hash);
  });

  // On Page Load
  openProjectBasedOnHash();
  ...
  function clearHash() {
    // http://stackoverflow.com/a/2295951/83916
  ...
  function openModal(elem) {
    ...
    $('#project-modal').modal({backdrop:false});
   }
  ...
  $('#project-modal').on('show.bs.modal', function () {
    ...
    if(elemDataCont.data('role')){
      $(this).find('#sdbr-role').show().text(elemDataCont.data('role'))
    }else{$(this).find('#sdbr-role').hide();}

    if(elemDataCont.data('link')) {
      if (elemDataCont.data('link').indexOf(",") >= 0) {
        var extLink = elemDataCont.data('link').split(',');

        //$(this).find('#sdbr-link').show().find('a').text(extLink[0]).attr('href', extLink[1]);
        $(this).find('#sdbr-link').show().html('<a href="'+extLink[1]+'" target="_blank" class="dont-break-out">'+extLink[0]+'</a>');
      } else {
        $(this).find('#sdbr-link').show().html('<span>Internal facing site</span>');
      }
